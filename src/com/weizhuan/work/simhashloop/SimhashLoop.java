package com.weizhuan.work.simhashloop;

import com.huaban.analysis.jieba.JiebaSegmenter;
import com.huaban.analysis.jieba.JiebaSegmenter.SegMode;
import com.huaban.analysis.jieba.SegToken;
import com.weizhuan.work.simhash.Murmur3;
...
//    private int fracCount = 4; // 默认按照4段进行simhash存储
    ...
    public SimhashLoop(int fracCount, int hammingThresh) {
...
//            System.out.println("tomcat frac:"+frac);
            Map<String, List<Long>> fracMap = storage.get(i);
//            System.out.println("tomcat storage size:"+storage.size());
            if (fracMap.containsKey(frac)) {
                for (Long simhash2 : fracMap.get(frac)) {
                    if (hamming(simhash, simhash2) < hammingThresh) {
                        String sourceCon = SimhashUtilMain.mSimHashContentMap.get(simhash2);
//                        String sourcePath = Main.contentPathMap.get(sourceCon);
                        List<String> l1 = SimhashUtilMain.mContentPathMap.get(sourceCon);
                        String targetCon = content;
//                        String targetPath = Main.contentPathMap.get(targetCon);
                        List<String> l2 = SimhashUtilMain.mContentPathMap.get(targetCon);
                        ...
                        SimhashUtilMain.mPaperSimResult.add(ls);
                        return true;
                    }
                }
            }
...
//        System.out.println("tomcat dis:"+dis);
        return dis;
    }
